...
> 專案內容本身請看 [`導覽.md`](導覽.md)（總入口）或 [`01-企劃/IP總覽.md`](01-企劃/IP總覽.md)。
> 這份 README 只講版控。
...
- 所有文件：`01-企劃/`（`IP總覽.md`、`對標計畫.md`、`發布資訊-第一支.md`、`主題提案-5支.md`…）、
  `02-技術選型/`、`03-歷史交接/`、`接手-下次開機.md`、
  `character/角色設定.md`、`video-01/腳本.md`、`video-01/進度.md`

> 2026-08-11 整理過一次目錄：根目錄的 .md 依用途收進 `01-企劃/`、`02-技術選型/`、`03-歷史交接/`，
> `auto/` 的診斷截圖、一次性 log、`.bak` 收進 `_診斷截圖/`、`_log/`、`_舊版備份/`。
> **沒有刪任何檔案**，被腳本引用的路徑（`auto/`、`待審核/`、`sfx/lib`、`character/`、
> `每日體檢/`、`channel-art/`、根目錄的 `taco-*.mp4` 與頻道視覺 png）一律留在原位沒動。
