...
## 發布前一定要人工過目（2026-08-08 新規則）

賢賢定的鐵律：**本機 ComfyUI 的片子不准發布，發布只能用 Veo；而且發之前一定先給賢賢看過。**

起因：8/8 那天 Veo 額度掛掉，舊版產線自動退回本機生成並直接發布，結果
D1S1 影片後段憑空多長出第二隻哈士奇、Taco 的黑點眉和藍項圈也不見了。

新流程：

```
場景圖(Gemini) → 影片(Veo) → 待審核\ → 賢賢看過 → ok → 發布
                     ↓ 失敗
                 等 1 小時再試（最多 8 次），不碰本機、不發布
```
